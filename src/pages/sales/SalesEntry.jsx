...
const SalesEntry = () => {
  ...
  return (
    <div className="flex flex-col gap-6 p-2 lg:p-6 bg-gray-50/50 dark:bg-gray-900 min-h-screen text-gray-800 dark:text-gray-100 font-sans">
               ...
               <div className="flex flex-col gap-6">
                 ...
                 {filteredInventory.length > itemsPerPage && (
                   <div className="flex justify-between items-center bg-white dark:bg-gray-800 p-4 rounded-2xl border border-gray-100 dark:border-gray-700">
                      <p className="text-[10px] font-black text-gray-400 uppercase">Page {currentPage} of {totalPages}</p>
                      <div className="flex gap-2">
                        <button onClick={() => goToPage(currentPage - 1)} disabled={currentPage === 1} className="p-2 border rounded-xl disabled:opacity-30"><ChevronLeft size={16}/></button>
                        <button onClick={() => goToPage(currentPage + 1)} disabled={currentPage === totalPages} className="p-2 border rounded-xl disabled:opacity-30"><ChevronRight size={16}/></button>
                      </div>
                   </div>
                 )}
               </div>
      ...
      <div className="w-full bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-xl overflow-hidden flex flex-col lg:flex-row min-h-[400px]">
          <div className="flex-1 border-r border-gray-100 dark:border-gray-700 flex flex-col">
               <div className="p-5 border-b border-gray-100 dark:border-gray-700">
                    <h2 className="text-xl font-bold flex items-center gap-2"><ShoppingCart className="text-primary"/> Current Order</h2>
                    <div className="relative mt-4">
                        <div className="bg-gray-50 dark:bg-gray-750 rounded-xl p-1 flex items-center border border-gray-200 dark:border-gray-600 focus-within:border-primary">
                            <div className="p-2.5 text-gray-400"><User size={18} /></div>
                            <input 
                                type="text" 
                            ...
                            />
                            {selectedCustomer && (
                                <button onClick={() => {setSelectedCustomer(null); setCustomerSearch('');}} className="p-2 text-gray-400 hover:text-red-500">
                                    <XCircle size={16} />
                                </button>
                            )}
                        </div>
                                    ...
                                    filteredCustomers.map(c => (
                                        <div 
                                            ...
                                            onClick={() => {
                                                ...
                                                setShowCustomerDropdown(false);
                                                // Auto-fill patient details from customer
                                                setPatientDetails(prev => ({
                                                    ...prev,
                                                    name: c.name,
                                                    mobile: c.phone || '',
                    ...
                    <div className="pt-3 mt-3 border-t border-gray-100 dark:border-gray-700">
                        <button 
                            ...
                            className="w-full text-xs font-bold text-gray-500 flex items-center justify-between hover:text-primary transition-colors bg-gray-50 dark:bg-gray-750 p-2 rounded-lg"
                        >
                            <span>{showExtraDetails ? 'Hide Extra Details' : '+ Add Patient & Shipping Info'}</span>
                            {showExtraDetails ? <ChevronUp size={14}/> : <ChevronDown size={14}/>}
                        </button>
                        
                        {showExtraDetails && (
                            <div className="mt-2 p-3 rounded-xl bg-white dark:bg-gray-800 border dark:border-gray-700 shadow-inner text-xs space-y-3">
                                <div className="grid grid-cols-2 gap-2">
                                    <div>
                                        <label className="block text-[10px] text-gray-400 uppercase font-bold mb-1">Patient Name</label>
                                        <input value={patientDetails.name} onChange={e => setPatientDetails({...patientDetails, name: e.target.value})} className="w-full p-2 rounded border dark:bg-gray-700 dark:border-gray-600 outline-none focus:border-primary" />
                                    </div>
                                    <div>
                                        <label className="block text-[10px] text-gray-400 uppercase font-bold mb-1">Mobile</label>
                                        <input value={patientDetails.mobile} onChange={e => setPatientDetails({...patientDetails, mobile: e.target.value})} className="w-full p-2 rounded border dark:bg-gray-700 dark:border-gray-600 outline-none focus:border-primary" />
                                    </div>
                                </div>
                                <div className="grid grid-cols-2 gap-2">
                                    <div>
                                        <label className="block text-[10px] text-gray-400 uppercase font-bold mb-1">Age</label>
                                        <input value={patientDetails.age} onChange={e => setPatientDetails({...patientDetails, age: e.target.value})} className="w-full p-2 rounded border dark:bg-gray-700 dark:border-gray-600 outline-none focus:border-primary" />
                                    </div>
                                    <div>
                                        <label className="block text-[10px] text-gray-400 uppercase font-bold mb-1">Gender</label>
                                        <select value={patientDetails.gender} onChange={e => setPatientDetails({...patientDetails, gender: e.target.value})} className="w-full p-2 rounded border dark:bg-gray-700 dark:border-gray-600 outline-none focus:border-primary">
                                ...
                                <div>
                                    ...
                                    <input value={patientDetails.address} onChange={e => setPatientDetails({...patientDetails, address: e.target.value})} className="w-full p-2 rounded border dark:bg-gray-700 dark:border-gray-600 outline-none focus:border-primary" />
                                </div>
                                <div className="grid grid-cols-2 gap-2">
                                    <div>
                                        <label className="block text-[10px] text-gray-400 uppercase font-bold mb-1">Doctor Name</label>
                                        <input value={patientDetails.doctorName} onChange={e => setPatientDetails({...patientDetails, doctorName: e.target.value})} className="w-full p-2 rounded border dark:bg-gray-700 dark:border-gray-600 outline-none focus:border-primary" />
                                    </div>
                                    <div>
                                        <label className="block text-[10px] text-gray-400 uppercase font-bold mb-1">Doctor Address</label>
                                        <input value={patientDetails.doctorAddress} onChange={e => setPatientDetails({...patientDetails, doctorAddress: e.target.value})} className="w-full p-2 rounded border dark:bg-gray-700 dark:border-gray-600 outline-none focus:border-primary" />
                                    </div>
                                </div>
                                
                                <div className="border-t border-gray-100 dark:border-gray-700 pt-2 mt-2">
                                    <p className="font-bold text-primary mb-2 flex items-center gap-1"><Package size={12}/> Shipping Label</p>
                                    <div className="flex gap-4 mb-2">
                                        <label className="flex items-center gap-1 cursor-pointer"><input type="radio" name="pack" checked={shippingDetails.packingType === 'Box'} onChange={() => setShippingDetails({...shippingDetails, packingType: 'Box'})}/> Box</label>
                                        <label className="flex items-center gap-1 cursor-pointer"><input type="radio" name="pack" checked={shippingDetails.packingType === 'Poly'} onChange={() => setShippingDetails({...shippingDetails, packingType: 'Poly'})}/> Poly</label>
                                        <label className="flex items-center gap-1 cursor-pointer ml-auto"><input type="checkbox" checked={shippingDetails.isColdStorage} onChange={e => setShippingDetails({...shippingDetails, isColdStorage: e.target.checked})}/> <span className="text-blue-500 font-bold">Cold ❄️</span></label>
                                    </div>
                                    <div className="grid grid-cols-2 gap-2">
                                        <div>
                                            <label className="block text-[10px] text-gray-400 uppercase font-bold mb-1 text-[8px]">Box Count</label>
                                            <input type="number" value={shippingDetails.boxCount} onChange={e => setShippingDetails({...shippingDetails, boxCount: Number(e.target.value)})} className="w-full p-1.5 rounded border dark:bg-gray-700 dark:border-gray-600 outline-none" />
                                        </div>
                                        <div>
                                            <label className="block text-[10px] text-gray-400 uppercase font-bold mb-1 text-[8px]">Poly Count</label>
                                            <input type="number" value={shippingDetails.polyCount} onChange={e => setShippingDetails({...shippingDetails, polyCount: Number(e.target.value)})} className="w-full p-1.5 rounded border dark:bg-gray-700 dark:border-gray-600 outline-none" />
                                        </div>
                                    </div>
                                </div>
                            </div>
               ...
               <div className="flex-1 overflow-y-auto p-4">
                    {cart.length === 0 ? <div className="text-center text-gray-400 py-20">Cart is empty</div> : (
                                        ...
                                        <tr key={item.id} className="border-b last:border-0">
                                             ...
                                             <td className="py-3 text-center">
                                                  <div className="flex justify-center gap-2">
                                                       <button onClick={() => updateQty(item.id, -1)} className="p-1 bg-gray-100 rounded"><Minus size={10}/></button>
                                                       <span className="text-xs">{item.qty}</span>
                                                       <button onClick={() => updateQty(item.id, 1)} className="p-1 bg-gray-100 rounded"><Plus size={10}/></button>
                                                  </div>
                                             </td>
                                             <td className="py-3 text-right text-xs font-bold">₹{item.amount.toFixed(2)}</td>
                                             <td className="py-3 text-right"><button onClick={() => removeFromCart(item.id)} className="text-red-400"><Trash2 size={14}/></button></td>
                                        </tr>
                                   ))}
                              </tbody>
                         </table>
          ...
          <div className="w-full lg:w-[350px] p-6 bg-gray-50/30 dark:bg-gray-900/10 flex flex-col justify-between">
               <div className="space-y-4">
                    <div className="flex justify-between text-sm"><span>Subtotal</span><span className="font-bold">₹{subTotal.toFixed(2)}</span></div>
                    <div className="flex justify-between text-sm"><span>Tax</span><span className="font-bold">₹{totalTax.toFixed(2)}</span></div>
               ...
               <div className="space-y-3 mt-6">
                    <div className="mb-4">
                        <label className="text-[10px] uppercase font-bold text-gray-400 mb-1 block">Payment Method</label>
                        <select 
                            value={paymentMode} 
                            onChange={(e) => setPaymentMode(e.target.value)}
                            className="w-full bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl px-3 py-2 text-sm font-bold outline-none focus:border-primary transition-colors"
                        >
                    ...
                    <div className="flex gap-2">
                         <button onClick={clearCart} className="p-3 border rounded-xl text-red-500 hover:bg-red-50"><Trash2 size={20}/></button>
                         <button onClick={handleProcessSale} disabled={cart.length === 0} className="flex-1 bg-primary text-white font-bold py-3 rounded-xl shadow-lg shadow-primary/30 disabled:opacity-50">Process Payment</button>
                    </div>
               </div>
          </div>
      </div>
